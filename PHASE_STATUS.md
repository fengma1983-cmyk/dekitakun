...
> **次回セッション開始時の最初の作業**: S3+S4 残りの本体（漢字化 messages.ts + CurrentTaskPanel）または S5（テスト自動化）に着手。
> 具体的な選択は当該セッションで決定。リネームは 2026-06-07 に完了済。
...
| **Phase 1-α** | 最小可動版: 加タスク · 完了 · 祝福 | 🟢 **今ここ (バッチ B + リネーム 完了、残: 漢字化)** |
...
- [x] **リネーム: 「やめる」→「閉じる」、「けす」→「消す」** — AddTaskModal.tsx の 2 行のテキスト改名（67e09d5 で完了、2026-06-07）。TaskDetailPopover は元から「消す」「閉じる」を使用済（旧描述「AddTaskModal と TaskDetailPopover 内」は誤記）。
